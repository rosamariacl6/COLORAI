...
import "./allProfessional.scss";
import { Avatar } from "@mui/material";
import { NavbarAdmin } from "../../../components/navbarAdmin/NavbarAdmin";
export const AllProfessional = () => {
  const navigate = useNavigate();
  const [user, setProfessional] = useState([]);
  const [users, setUsers] = useState();
  useEffect(() => {
    axios
      .get("http://localhost:4000/users/allProfessional")
      .then((res) => {
        console.log(res);
  ...
  const delProfessional = (id) => {
    axios
      .delete(`http://localhost:4000/users/deleteUser/${id}`)
      .then((res) => {
        console.log(res);
        setProfessional(user.filter((elem) => elem.user_id !== id));
      })
      .catch((err) => {
        console.log(err);
      });
                ...
                <Col xs={12} md={6} lg={3} key={user.user_id}>
                  <Card className="mb-4 text-center user-card">
                    <Card.Body>
                      ...
                      <Button
                        variant="danger"
                        onClick={() => delProfessional(user.user_id)}
                      >
                        Borrar
                      </Button>
                      &nbsp;&nbsp;
                      <Button
                        variant="warning"
                        onClick={() =>
                          navigate(`/editAdminProfessional/${user.user_id}`)
                        }
                      >
                        Editar
                      </Button>
